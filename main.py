...
staff_lower, staff_upper, staff_thresh = 50, 110, 0.70
sharp_lower, sharp_upper, sharp_thresh = 100, 100, 0.68
flat_lower, flat_upper, flat_thresh = 100, 100, 0.68
quarter_lower, quarter_upper, quarter_thresh = 50, 110, 0.65
half_lower, half_upper, half_thresh = 50, 110, 0.63
whole_lower, whole_upper, whole_thresh = 100, 100, 0.70
...
#   img = img[int(y*0.15):int(y*0.4),190:x-120,:]
#   cv2.imwrite('cut.png',img)
	...
	cv2.imwrite('test_pre.png',img_gray)
	...
	# last_line = max(heights)-40
	# testing = img_gray[last_line:,:]
	# cv2.imwrite('testing.png', testing)
	# open_file('testing.png')
	...
	for note_group in note_groups:
		for note in note_group:
			save_pitch.append(note.note[:1])

	print(len(save_pitch))
	for pitch in save_pitch:
		print(pitch)
	
	print('cost time: {:.0f}:{:.0f}'.format((time.time()-start_time)/60,(time.time()-start_time)%60))
...
'''
	...
	midi.addTrackName(track, time_midi, "Track")
	midi.addTempo(track, time_midi, 140)
	...
	for note_group in note_groups:
		...
		for note in note_group:
			...
			pitch = note.pitch
			midi.addNote(track,channel,pitch,time_midi,duration,volume)
			time_midi += duration

	midi.addNote(track,channel,pitch,time_midi,4,0)
